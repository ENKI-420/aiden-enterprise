...
function AlertItem({
  ...
  const getTypeStyles = () => {
    switch (type) {
      case "info":
        return {
          icon: Info,
          iconClasses: "bg-blue-500/10 border-blue-500/30 text-blue-500",
        }
      case "warning":
        return {
          icon: AlertCircle,
          iconClasses: "bg-amber-500/10 border-amber-500/30 text-amber-500",
        }
      case "error":
        return {
          icon: AlertCircle,
          iconClasses: "bg-red-500/10 border-red-500/30 text-red-500",
        }
      case "success":
        return {
          icon: Check,
          iconClasses: "bg-green-500/10 border-green-500/30 text-green-500",
        }
      case "update":
        return {
          icon: Download,
          iconClasses: "bg-cyan-500/10 border-cyan-500/30 text-cyan-500",
        }
      default:
        return {
          icon: Info,
  ...
  const { icon: Icon, iconClasses } = getTypeStyles()

  return (
    <div className="flex items-start space-x-3">
      <div className={`mt-0.5 p-1 rounded-full ${iconClasses}`}>
